...
export default function AddTripScreen({ navigation }: any) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // 2. Tạo record Trip mới (khớp với schema mới)
      const newTrip = {
        user_id: user.uid,
        title: generatedData.title || `Khám phá ${data.destination}`,
        description: generatedData.summary || 'Lịch trình được tạo bởi AI',
        country: generatedData.country || 'Việt Nam',
        city: data.destination,
        start_date: new Date().toISOString().split('T')[0],
        end_date: new Date(Date.now() + parseInt(data.days) * 86400000).toISOString().split('T')[0],
        budget: parseInt(data.budget),
        total_distance: generatedData.distance || Math.floor(Math.random() * 500) + 50,
        total_cost: 0,
        status: 'Upcoming',
